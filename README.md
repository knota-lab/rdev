...
这样前台进程名是 `rdev-dev.exe`，它启动的后台 daemon 是 `rdev-dev-daemon.exe`。正式安装版仍然使用 `rdev.exe`，后台 daemon 是 `rdev-installed-daemon.exe`，在任务管理器里更不容易误杀。
